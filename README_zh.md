...
TerAgent 是一个用于构建生产级 AI Agent 系统的 Python 框架，提供从**多 Agent 编排**（Swarm/Sequential 模式与 Handoff 协作）到**工具编排**（并行/串行调度与权限管控），从**7 层安全体系**到**4 层可靠性熔断**，从 **TAP IR** 模型无关编译到**智能多模型路由**的全栈能力。

### 核心支柱

| 支柱 | 解决的问题 | 核心机制 |
|---|---|---|
| **多 Agent 编排** | 单 Agent 无法应对复杂工作流 | Swarm 模式（Handoff 协作）、Sequential 模式、SharedState 共享状态、CancellationToken 取消令牌、生命周期钩子 |
| **工具编排** | 无管控的工具执行既不安全又低效 | 并行/串行智能调度、权限分级、Hook 集成、安全感知分区 |
| **安全体系** | 拥有工具访问权的 Agent 需要护栏 | 7 层权限裁决、6 层命令防御、2PC 文件写入、3 级沙箱 |
| **可靠性** | Agent 在无限循环和失败中消耗 Token | 4 种熔断器、流式重试与批量回退、步数预算、上下文压缩 |
| **多模型** | 没有单一模型适合所有任务；各厂商格式不同 | TAP IR（模型无关）、9 编译器 × 5 适配器 = 45 种组合、6 步智能路由 |
| **自我进化** | 缺乏结构化方式捕获 Agent 交互用于改进 | TAPTracer 记录每次请求→响应并生成 DPO 偏好对 |
...
📖 **相关文档：**
- [完整文档目录](docs/zh/index.md) — 指南、API 参考、配置手册
...
| Windows 系统路径保护 | ✅ `C:\Windows`、`System32`、`System` | N/A | N/A |
...
4. 通过 Orchestrator 委派 Agent（若 CREATE_PROJECT + 多 Agent 配置）
5. 工具循环：
   a. 检查步骤预算
...
| 多 Agent 编排 | `Orchestrator` | 5 种编排模式的多 Agent 协调 |
...
第 5 层: PermissionLevel 检查       ← DEFAULT / PLAN / BYPASS / ACCEPT_EDITS / AUTO / CUSTOM
...
`Orchestrator` 通过可插拔的执行模式协调多个 Agent：

| 模式 | 行为 | 使用场景 |
|------|------|----------|
| `Sequential` | 按顺序执行 Agent（A→B→C） | 每步依赖前一步的流水线任务 |
| `Swarm` | Agent 通过 `transfer_to_{agent}` 工具调用交接控制权 | 动态路由，Agent 自行决定谁来处理下一步 |
| `Parallel` | 扇出→扇入，所有 Agent 并发运行 | 可同时运行的独立子任务 |
| `Conditional` | 路由 Agent 决定交接给哪个 Agent | 基于输入类型或内容的任务路由 |
| `Loop` | 生成器-评审器循环，支持退出条件 | 迭代优化直到达到质量阈值 |

**核心概念：**
- **Agent** — 独立单元，拥有自己的 provider、工具集、handoff、guardrail、hooks
- **Handoff** — Swarm 模式下 Agent 间的控制权转移，支持输入过滤
- **SharedState** — 跨 Agent 的作用域状态共享（session / agent / global）
- **CancellationToken** — 线程安全协作式取消，用于长时间编排
- **Guardrail** — 输入/输出护栏，fail-fast 并行检查引擎
- **ApprovalGate** — Human-in-the-loop 工具审批（支持参数修改）
- **AgentHooks** — 生命周期钩子（on_start/end/handoff/tool/model）
- **OrchestratorTool** — 嵌套编排：将整个 Orchestrator 包装为工具
...
| `teragent.config.orchestration_config` | `OrchestrationConfig` | 多 Agent 编排设置（5 种模式、Agent 配置） |
| `teragent.config.agent_config` | `AgentConfig` | 单个 Agent 定义（provider、工具、handoff） |
| `teragent.config.mcp_config` | `MCPServerConfig` | MCP 服务器连接参数 |
...
- 事件历史：追踪最近 100 个基础事件 + 200 个结构化事件，用于调试
...
TerAgent 完全通过 AI 辅助开发构建——所有代码均由 AI 在人类指导下生成。项目遵循**设计 → 规划 → 编码 → 审查**流水线：

### Phase 1：基础框架（W1–W4）

建立了核心的多 Agent 编排框架和工具系统：

- **编排引擎** — `Orchestrator` 采用策略模式，支持 `Sequential` 和 `Swarm` 两种执行模式；`Agent` 基类支持独立 provider、工具集、handoff 和 guardrail；`Handoff` + `HandoffTool` 实现 Swarm 风格的控制转移；`SharedState` 提供跨 Agent 状态共享（session/agent/global 三级作用域）；`CancellationToken` 实现协作式取消；`AgentHooks` 提供生命周期观测。
- **工具系统** — `@tool` 装饰器将 Python 函数转换为 `BaseTool`，自动生成 JSON Schema；`AgentTool` 实现 Agent-as-Tool 委托模式；9 个内置工具（文件读写、代码执行、网络搜索/抓取、代码分析）。
- **Agent 与编排配置** — `AgentConfig` 和 `OrchestrationConfig` 支持声明式 TOML 配置。

### Phase 2：协议集成与高级模式（W5–W8）

集成外部工具协议，添加高级编排模式：

- **MCP 集成** — `MCPToolset` 支持 stdio/SSE/streamable_http 三种传输；`MCPConnectionPool` 提供 LRU 驱逐和健康检查的连接池。
- **OpenAPI 工具集** — 从 OpenAPI 3.0 / Swagger 2.0 规范自动生成工具，含安全级别推断。
- **ToolPack** — 分组相关工具，共享生命周期（`on_start`/`on_stop`）和资源管理。
- **工具市场** — 搜索、安装、发布远程工具的市场客户端。
- **高级模式** — `ParallelPattern`（扇出/扇入）、`ConditionalPattern`（路由器模式）、`LoopPattern`（生成器-评审者模式，支持退出条件）。
- **增强注册表** — 分类注册、意图推荐、`ToolInfo` 元数据。

### Phase 3：安全与生产就绪（W9–W12）

为生产环境加固系统：

- **认证体系** — 统一的 `AuthManager`，支持 bearer / API key / OAuth2 / basic 认证方案；`AuthCredential` 安全内存存储，`__repr__` 掩码敏感字段。
- **嵌套编排** — `OrchestratorTool` 实现层级式多 Agent 工作流。
- **结果缓存** — `ResultCache` 支持 TTL + LRU 驱逐、确定性键生成、命中/未命中/淘汰统计。
- **检查点与恢复** — `OrchestrationCheckpoint` 原子写入（tempfile + `os.fsync` + `os.replace`），保存/恢复/列表/清理。
- **人机协同** — `ApprovalGate` 提供批准/拒绝/修改参数工作流和超时处理。
- **异步读写锁** — 写者优先的 `AsyncRWLock`，保障 `SharedState` 的并行安全访问。

### 开发方法论

这条流水线本身也是 TerAgent 的一部分：`pipeline` 模块提供了可复用的**设计 → 规划 → 编码 → 审查**流程，正是用此流程构建了项目本身。
